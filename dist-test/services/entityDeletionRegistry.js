...
class EntityDeletionRegistry {
    ...
    async executeWithoutTransaction(context) {
        ...
        try {
            ...
            const validation = await this.executeWithRetry(() => strategy.validate(context), `validation for ${context.entityType}`, ((_a = context.config) === null || _a === void 0 ? void 0 : _a.maxRetries) || this.MAX_RETRY_ATTEMPTS);
            if (!validation.isValid) {
                return {
                    success: false,
                    deletedEntities: [],
            ...
            const result = await this.executeWithRetry(() => strategy.execute(context), `deletion for ${context.entityType}`, ((_b = context.config) === null || _b === void 0 ? void 0 : _b.maxRetries) || this.MAX_RETRY_ATTEMPTS);
     ...
     */
    async executeWithRetry(operation, operationName, maxRetries = this.MAX_RETRY_ATTEMPTS) {
        let lastError;
...
//# sourceMappingURL=entityDeletionRegistry.js.map