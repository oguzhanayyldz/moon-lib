...
class OptimisticLockingUtil {
    ...
    */
    static saveWithRetry(document, operationName, session, reapply) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield this.retryWithOptimisticLocking(() => __awaiter(this, void 0, void 0, function* () {
                attempt++;
                const target = attempt === 1 ? document : yield this.reloadDocument(document, boundSession);
                if (attempt > 1) {
                    yield reapply(target);
                }
                const saveOptions = boundSession ? { session: boundSession } : {};
                yield target.save(saveOptions);
                return target;
     ...
     * @private
     */
    static reloadDocument(document, session) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const fresh = yield (session ? query.session(session) : query);
            if (!fresh) {
    ...
    */
     ...
     */
    static publishVersionEvent(Model_1, doc_1) {
        return __awaiter(this, arguments, void 0, function* (Model, doc, options = {}) {
            var _a, _b, _c, _d;
            ...
            try {
                return yield this.publishVersionEventForUpdate(Model, doc, version, (_c = options.source) !== null && _c !== void 0 ? _c : 'explicit', options.session);
            }
            catch (error) {
     ...
     * @private
     */
    static publishVersionEventForUpdate(Model_1, doc_1, newVersion_1) {
        return __awaiter(this, arguments, void 0, function* (Model, doc, newVersion, source = 'updateWithRetry', session) {
            var _a, _b;
            ...
            if (session) {
                yield Outbox.create([outboxPayload], { session });
            }
            else {
    ...
    */
    static applyVersionedUpdate(Model, id, version, updateFields, operationName) {
        return __awaiter(this, void 0, void 0, function* () {
            const docName = operationName || `${Model.modelName} ${id}`;
            /**
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                ...
                const doc = yield Model.findOneAndUpdate({ _id: id, version: { $lt: version } }, { $set: Object.assign(Object.assign({}, updateFields), { version, updatedOn: new Date() }) }, { new: true });
                if (doc) {
                    try {
                        // updateMetadataWithRetry ile AYNI yayın yolu: sync servisi
                        // (EntitySyncState/versionDiff) kopyanın ilerlediğini görmeli.
                        yield this.publishVersionEventForUpdate(Model, doc, version);
                    }
                    catch (error) {
                        logger_service_1.logger.error(`❌ Versiyon event'i yayınlanamadı (${docName}):`, error);
                ...
                const exists = yield Model.findOne({ _id: id, includeDeleted: true })
                    .select('_id')
                    .lean();
                if (!exists) {
