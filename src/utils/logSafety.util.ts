 ...
 */
export function maskConnectionUriSecret(value: string | undefined): string | undefined {
    if (!value) {
        return value;
    }
 ...
 */
export function maskConnectionUrisInText(text: string | undefined): string | undefined {
    if (!text) {
        return text;
    }
    let result = '';
 ...
 */
export function sanitizeConnectionError(error: unknown): SafeConnectionError {
    if (typeof error !== 'object' || error === null) {
        return { message: maskUriTextOrEmpty(String(error)) };
    }
    const source = error as Record<string, unknown>;
    const safeError: SafeConnectionError = {
 ...
 */
export function toSafeError(safe: SafeConnectionError): Error {
    const error = new Error(safe.message) as Error & { code?: string | number; input?: string; url?: string };
    if (safe.name) {
...
function maskUriTextOrEmpty(text: string): string {
    return maskConnectionUrisInText(text) ?? '';
}

function maskUriList(value: string): string {
    return value
...
function maskSingleUri(uri: string): string {
    const scheme = SCHEME_PATTERN.exec(uri)?.[0] ?? '';
...
function maskUserInfo(userInfo: string | undefined): string {
...
function maskTail(tail: string): string | undefined {
    if (tail.includes('#')) {
        return undefined;
    }
...
function maskQueryParam(param: string): string {
 ...
 */
export function maskSensitiveValues(value: unknown, depth = 0): unknown {
    ...
    if (Array.isArray(value)) {
        const items: unknown[] = value.slice(0, MAX_SHAPE_ITEMS).map((item) => maskSensitiveValues(item, depth + 1));
...
function maskShapeKey(key: string): string {
...
export const REDACTED_FIELD_MASK = '***REDACTED***';
...
export function redactSensitiveFields<T>(value: T): T {
    ...
    return Object.fromEntries(
        Object.entries(value as Record<string, unknown>).map(([key, item]) => [
            key,
            isSensitiveFieldName(key) ? REDACTED_FIELD_MASK : redactSensitiveFields(item)
        ])
    ) as T;
}
...
function redactXmlElements(text: string): string {
    ...
    while ((match = startTag.exec(text)) !== null) {
        ...
        if ((attributes ?? '').endsWith('/') || !isSensitiveFieldName(localName)) {
            continue;
        }
        ...
        result += text.slice(copiedUpTo, contentStart) + REDACTED_FIELD_MASK;
...
function redactJsonPairs(text: string): string {
    return text.replace(JSON_PAIR_PATTERN, (pair, name: string, separator: string) =>
        isSensitiveFieldName(name) ? `"${name}"${separator}"${REDACTED_FIELD_MASK}"` : pair
    );
}
 ...
 */
export function maskResourceId(value: unknown): string {
    if (typeof value !== 'string') {
        return '****';
    }
 ...
 */
export function hashTenantId(value: unknown): string {
