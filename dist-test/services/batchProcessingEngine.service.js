...
class ConnectionPoolManager {
     ...
     */
    async waitForConnection(poolName) {
     ...
     */
    async removeConnection(poolName, connection) {
        const pool = this.pools.get(poolName);
        if (pool) {
...
class BatchQueueManager extends events_1.EventEmitter {
     ...
     */
    async addToQueue(queueName, items) {
...
class BatchProcessingEngine extends events_1.EventEmitter {
     ...
     */
    async processBatchDeletion(context) {
        ...
        try {
            ...
            for (const [entityType, items] of groupedItems) {
                try {
                    ...
                    const result = await this.processBatchGroup(strategy, items, context, operationId);
     ...
     */
    async processBatchGroup(strategy, items, context, operationId) {
        var _a;
        ...
        try {
            ...
            if (strategy.supportsParallel && batches.length > 1) {
                ...
                for (let i = 0; i < batches.length; i += maxConcurrent) {
                    ...
                    const batchResults = await Promise.allSettled(concurrentBatches.map(batch => strategy.processBatch(batch, context)));
...
//# sourceMappingURL=batchProcessingEngine.service.js.map