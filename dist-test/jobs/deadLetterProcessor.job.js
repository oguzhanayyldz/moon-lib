...
class DeadLetterProcessorJob {
     ...
     */
    async claimNextEvent(listenerKeys, processorId) {
    ...
    async processEvent(event) {
        ...
        if (result === 'processed') {
            await this.deadLetterModel.updateOne(this.claimedBy(event), {
                $set: {
                    status: 'completed',
     ...
     */
    async markReplayFailed(event, target) {
