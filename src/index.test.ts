...
export const createRedisWrapper = () => {
    ...
    return {
        client: {
            ...
            sAdd: jest.fn((key: string, member: string | string[]) => {
                if (!mockStorage[key]) mockStorage[key] = new Set<string>();
                if (!(mockStorage[key] instanceof Set)) mockStorage[key] = new Set<string>();
            ...
            sMembers: jest.fn((key: string) => {
                if (mockStorage[key] instanceof Set) {
                    return Promise.resolve(Array.from(mockStorage[key] as Set<string>));
                }
                return Promise.resolve([] as string[]);
            }),
            sRem: jest.fn((key: string, member: string | string[]) => {
                if (!(mockStorage[key] instanceof Set)) return Promise.resolve(0);
                const members = Array.isArray(member) ? member : [member];
                let removed = 0;
                for (const m of members) {
                    if (mockStorage[key].delete(m)) removed++;
                }
                return Promise.resolve(removed);
            }),
            sCard: jest.fn((key: string) => {
                if (mockStorage[key] instanceof Set) return Promise.resolve(mockStorage[key].size);
            ...
            eval: jest.fn((script: string, options?: { keys?: string[], arguments?: any[] }) => {
...
const mockSaveWithRetry = async (doc: any, _operationName?: string, session?: any, reapply?: (fresh: any) => any) => {
    ...
    for (let attempt = 1; ; attempt++) {
        try {
            ...
            const fresh = await (session && typeof query?.session === 'function' ? query.session(session) : query);
            if (!fresh) {
...
export const OptimisticLockingUtil = {
    ...
    updateMetadataWithRetry: jest.fn().mockImplementation(
        async (model, id, updateData, options = {}) => {
            const result = await model.findByIdAndUpdate(
                id,
                updateData,
    ...
    applyVersionedUpdate: jest.fn().mockImplementation(
        async (Model: any, id: string, version: number, updateFields: Record<string, any>, _operationName?: string) => {
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                ...
                if (doc) return { outcome: 'applied', doc };
                ...
                const query: any = Model.findOne({ _id: id, includeDeleted: true });
                const exists = (query && typeof query.select === 'function')
                    ? await query.select('_id').lean()
                    : await query;
                if (!exists) return { outcome: 'missing', doc: null };
...
export const setupTestEnvironment = () => {
    ...
    OptimisticLockingUtil.updateMetadataWithRetry = jest.fn().mockImplementation(
        async (model: any, id: string, updateData: any, options: any = {}) => {
            const result = await model.findByIdAndUpdate(
                id,
                updateData,
                { new: true, omitUndefined: true, ...options }
