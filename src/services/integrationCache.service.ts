 ...
 * ```
 */
export class IntegrationCacheService<T extends object> {
    ...
    constructor(serviceName: string, options: {
        ...
        this.cache = new LRUCache<string, T>({
            ...
            dispose: (value: T, key: string, reason: LRUCache.DisposeReason) => {
                logger.info(`[${this.serviceName}] Integration instance evicted from cache`, {
                    key,
                    reason,
        ...
        logger.info(`[${this.serviceName}] Integration cache initialized`, {
            maxSize: options.max,
            ttl: options.ttl,
    ...
    get(key: string): T | undefined {
        ...
        if (instance) {
            logger.debug(`[${this.serviceName}] Cache HIT`, {
                key,
                cacheSize: this.cache.size,
                remainingTTL: this.cache.getRemainingTTL(key)
            });
        } else {
            logger.debug(`[${this.serviceName}] Cache MISS`, {
                key,
                cacheSize: this.cache.size
            });
        }
    ...
    set(key: string, value: T): void {
        ...
        logger.info(`[${this.serviceName}] Integration cached`, {
            key,
            cacheSize: this.cache.size,
            maxSize: this.cache.max,
            ttl: this.cache.ttl
    ...
    invalidate(key: string): boolean {
        ...
        if (existed) {
            logger.info(`[${this.serviceName}] Integration invalidated`, {
                key,
                cacheSize: this.cache.size
            });
        } else {
            logger.debug(`[${this.serviceName}] Invalidation requested but key not found`, {
                key
            });
        }
    ...
    invalidateUser(userId: string): number {
        ...
        if (count > 0) {
            logger.info(`[${this.serviceName}] User integrations invalidated`, {
                userId,
                count,
                cacheSize: this.cache.size
            });
    ...
    clear(): void {
        ...
        logger.warn(`[${this.serviceName}] Entire cache cleared`, {
            clearedCount: size
        });
    }
