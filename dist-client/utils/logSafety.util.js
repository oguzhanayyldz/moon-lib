...
function redactSensitiveText(text) {
    ...
    return redactHeaderPairs(redactFormPairs(redactJsonPairs(redactXmlElements(text))))
        .replace(JWT_PATTERN, exports.REDACTED_FIELD_MASK)
        .replace(URL_USERINFO_IN_TEXT_PATTERN, `$1${exports.REDACTED_FIELD_MASK}@`)
        .replace(BARE_BEARER_PATTERN, `$1 ${exports.REDACTED_FIELD_MASK}`);
}
...
function redactHeaderPairs(text) {
    var _a, _b;
    ...
    while ((match = headerName.exec(text)) !== null) {
        ...
        const quote = (_a = FORM_VALUE_QUOTE_PATTERN.exec(value)) === null || _a === void 0 ? void 0 : _a[0];
        const scheme = (_b = HEADER_AUTH_SCHEME_PATTERN.exec(value)) === null || _b === void 0 ? void 0 : _b[0];
        let masked;
        let valueLength;
        if (quote) {
            // Fail-closed: a quoted value that is never closed is masked up to the end of the text.
            const closingIndex = value.indexOf(quote, quote.length);
            valueLength = closingIndex === -1 ? value.length : closingIndex + quote.length;
            masked = quote + exports.REDACTED_FIELD_MASK + (closingIndex === -1 ? '' : quote);
        }
        else if (scheme) {
            valueLength = scheme.length;
            masked = `${scheme.split(/[ \t]/, 1)[0]} ${exports.REDACTED_FIELD_MASK}`;
        }
        else {
            const lineEnd = value.search(LINE_END_PATTERN);
            valueLength = lineEnd === -1 ? value.length : lineEnd;
            masked = exports.REDACTED_FIELD_MASK;
        }
        if (valueLength === 0) {
            continue;
        }
