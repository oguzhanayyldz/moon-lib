...
export const REDIS_KEY_ENV_VARIABLE = 'REDIS_KEY_ENV';
 ...
 */
export function getRedisKeyEnvironment(): string {
    return process.env[REDIS_KEY_ENV_VARIABLE] || process.env.NODE_ENV || PRODUCTION_ENVIRONMENT;
}

/** `true` when Redis names get an environment namespace (every resolved environment except production). */
export function isRedisEnvScoped(): boolean {
 ...
 */
export function envScopedKey(key: string): string {
 ...
 */
export function envScopedPattern(pattern: string): string {
    const environment = getRedisKeyEnvironment();
    if (environment === PRODUCTION_ENVIRONMENT) {
        return pattern;
    }
 ...
 */
export function stripEnvScope(key: string): string {
    const environment = getRedisKeyEnvironment();
    if (environment === PRODUCTION_ENVIRONMENT) {
        return key;
    }
 ...
 */
export function envBullPrefix(base: string = DEFAULT_BULL_PREFIX): string {
    const environment = getRedisKeyEnvironment();
    if (environment === PRODUCTION_ENVIRONMENT) {
        return base;
    }
